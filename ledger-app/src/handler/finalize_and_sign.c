...
int finalize_after_approval(void) {
     ...
     * derivation here is DUAL-derived too (AHW-020: az_derive_schnorr_signing_scalar
     * + the nonce each compute TWICE into independent buffers + a fault-hard compare,
     * aztec_secret.c M11 P1 — the dual-derive the older note called a "follow-up" is
     * DONE). A surviving glitch would still be fail-safe (a sig for a different key is
     * on-chain-invalid, never a spend break) + is independently cross-checked by the
     * pre-UI B3 (same scalar→address proved == consumer). Returns
     * s(32)||e_raw(32). The ECDSA-K
