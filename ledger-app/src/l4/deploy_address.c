...
int az_deploy_compute_partial_address(
     ...
     * Lay out the 2-field input contiguously per the regular hash API. */
    uint8_t init_payload[2 * 32];
    selector_to_fr_be(ctor_selector_u32, &init_payload[0]);
    memcpy(&init_payload[32], out_args_hash, 32);
    ...
    if (az_poseidon2_hash_with_separator(salted_payload, 3, L4_SEP_PARTIAL_ADDRESS, salted_init) != 0) {
     ...
     * Same separator as step 3 — that's deliberate, matches
     * contract_address.ts:35-49 (both partial-address chain steps reuse
     * the PARTIAL_ADDRESS separator). */
    uint8_t partial_payload[2 * 32];
    memcpy(&partial_payload[0], account_class_id, 32);
    memcpy(&partial_payload[32], salted_init, 32);
    explicit_bzero(salted_init, 32);

    if (az_poseidon2_hash_with_separator(partial_payload, 2, L4_SEP_PARTIAL_ADDRESS, out_partial_address) != 0) {
