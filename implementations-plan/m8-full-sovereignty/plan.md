...
- **`INS_GET_AZTEC_MASTER_SECRET` (0x12)** returns a 32-byte BE Fr. Internally: BIP-32 derive secp256k1 child at the requested path → `entropy_64 = SHA-512("aztec-master-secret-v1\0" || child_privkey_d(32))` → reduce mod BN254 scalar field (Aztec's `Fr` modulus) via wide reduction → return. **(As-built hashes the PRIVATE child key, not the pubkey — codex P4 BLOCKER: a pubkey-based digest is readable via GET_PUBLIC_KEY and would bypass the reveal gate. This bullet originally described the insecure pubkey variant; corrected M8 P7.0.)**
...
> **⚠ SUPERSEDED (M8 P7/8 owner reframe).** The backup/recovery design in the next three bullets is no longer the plan. Recovery is now **Ledger re-derivation** — plug in the device (or a Ledger restored from its own seed) → re-onboard → the same account/keys/balance come back; the deterministic salt makes the address recompute from the device, so there is **no BIP-39 mnemonic, no sidecar, no device-less read-only path**. See [phase-7-8-plan.md](phase-7-8-plan.md). Kept below for historical record.
