 ...
 *
 * Usage:
 *   bun run tools/clear-signing-codegen/gen-clear-signing-v0.ts          # write outputs
 *   bun run tools/clear-signing-codegen/gen-clear-signing-v0.ts --check  # CI drift-check
 */
...
// tools/clear-signing-codegen/ → repo root is two levels up.
const REPO_ROOT = resolve(__dirname, '../..');

/** Optional `--flag value` path override; falls back to the default. */
...
const MANIFEST_PATH = argPath('--manifest', resolve(__dirname, 'manifest.json'));

// Pinned artifacts, resolved from THIS tool's node_modules (it deps aztec-standards).
const TOKEN_ARTIFACT_PATH = resolve(
  __dirname,
  'node_modules/@defi-wonderland/aztec-standards/target/token_contract-Token.json',
);
...
// Codegen lives OUTSIDE the SDK; it writes into THREE trees (the TS split is P1's):
//   --out-c      firmware C tables        → ledger-app/src/clear_signing_v0
//   --out-ts     demo TS preflight tables → apps/demo-browser/src/clear-signing
//   --out-ts-sdk SDK TS deploy table      → packages/aztec-ledger/src/clear_signing_v0
const OUT_C_DIR = argPath('--out-c', resolve(REPO_ROOT, 'ledger-app/src/clear_signing_v0'));
const OUT_TS_DEMO_DIR = argPath(
  '--out-ts',
  resolve(REPO_ROOT, 'apps/demo-browser/src/clear-signing'),
);
const OUT_TS_SDK_DIR = argPath(
  '--out-ts-sdk',
  resolve(REPO_ROOT, 'packages/aztec-ledger/src/clear_signing_v0'),
);
 ...
 * Source-of-truth: tools/clear-signing-codegen/manifest.json
 ...
 * Single source of truth: tools/clear-signing-codegen/manifest.json
 */
...
async function main(): Promise<void> {
  ...
  const targets: { path: string; content: string }[] = [
    ...
    { path: join(OUT_TS_DEMO_DIR, 'registry.generated.ts'), content: regTs },
    { path: join(OUT_TS_DEMO_DIR, 'selectors.generated.ts'), content: selTs },
    { path: join(OUT_TS_SDK_DIR, 'deploy_profiles.generated.ts'), content: depTs },
  ];
