 ...
 *
 * Usage:
 *   bun run packages/aztec-ledger/scripts/gen-clear-signing-v0.ts          # write outputs
 *   bun run packages/aztec-ledger/scripts/gen-clear-signing-v0.ts --check  # CI drift-check
 */
...
const MANIFEST_PATH = resolve(REPO_ROOT, 'packages/aztec-ledger/clear-signing-v0/manifest.json');

const TOKEN_ARTIFACT_PATH = resolve(
  REPO_ROOT,
  'packages/aztec-ledger/node_modules/@defi-wonderland/aztec-standards/target/token_contract-Token.json',
);

const DRIPPER_ARTIFACT_PATH = resolve(
  REPO_ROOT,
  'packages/aztec-ledger/node_modules/@defi-wonderland/aztec-standards/target/dripper-Dripper.json',
);

const OUT_C_DIR = resolve(REPO_ROOT, 'ledger-app/src/clear_signing_v0');
const OUT_TS_DIR = resolve(REPO_ROOT, 'packages/aztec-ledger/src/clear_signing_v0');
 ...
 * Source-of-truth: packages/aztec-ledger/clear-signing-v0/manifest.json
